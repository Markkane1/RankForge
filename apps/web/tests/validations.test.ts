...
import { isSafeExternalUrl, safeUrlSchema, createTaskSchema } from '../src/lib/validations';
...
describe('Zod Validations & SSRF Prevention (REQ-03, REQ-06)', () => {
  describe('SSRF Protection (isSafeExternalUrl)', () => {
    it('should reject local/private IP addresses', () => {
      ...
      expect(isSafeExternalUrl('http://169.254.169.254/latest/meta-data')).toBe(false); // AWS Metadata
