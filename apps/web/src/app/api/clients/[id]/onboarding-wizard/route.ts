  ...
  try {
    ...
    if (hasSite) {
      if (!hasCmsLogin) {
        ...
        newState = "GROWTH";
        taskTitle =
          "Perform landing page SEO optimization audit & schema integration";
      ...
      if (wantsSite) {
        ...
        newState = "BUILD";
        taskTitle =
          "Draft location page template & design mockup for new website";
        ...
        newState = "GROWTH";
        taskTitle =
          "Configure off-site Google Business Profile primary content and citations";
    ...
    const { task, updatedClient } = await withClientTenant(
      id,
      async (tenantDb) => {
        ...
        const updatedClient = await transitionClientTo(
          id,
          newState,
          auth.user.id,
          tenantDb,
