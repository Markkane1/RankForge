        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div className="bg-white dark:bg-slate-850 p-6 rounded-xl border shadow-sm flex flex-col justify-between">
            ...
            <div className="mt-2 flex items-baseline">
              <span className="text-2xl font-bold text-emerald-600">
                {baselineData
                  ? `${baselineData.initialAverageRank.toFixed(1)}`
                  : "—"}
              </span>
          ...
          <div className="bg-white dark:bg-slate-850 p-6 rounded-xl border shadow-sm flex flex-col justify-between">
            ...
            <span className="text-xs text-slate-400 mt-2">
              Latest Scan:{" "}
              {scans[0]
                ? new Date(scans[0].scanDate).toLocaleDateString()
                : "—"}
            </span>
          </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <section className="bg-white dark:bg-slate-850 p-6 rounded-xl border shadow-sm">
          ...
          <div className="space-y-3">
            {leads.length === 0 ? (
              <p className="text-sm text-slate-400">No tracked leads yet.</p>
            ) : (
              leads.map((lead) => (
                <div
                  key={lead.id}
              ...
              approvals.map((approval) => (
                <div key={approval.id} className="p-3 border rounded-lg">
                  ...
                  <p className="mt-2 text-xxs text-slate-400">
                    {approval.requestType.replaceAll("_", " ")} -{" "}
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <section className="bg-white dark:bg-slate-850 p-6 rounded-xl border shadow-sm">
          ...
          <div className="space-y-3">
            {reports.length === 0 ? (
              <p className="text-sm text-slate-400">
                No monthly reports finalized yet.
              </p>
            ) : (
              reports.map((report) => (
                <div
                ...
                >
                  <div className="flex flex-col">
                    <span className="text-sm font-semibold">
                      SEO Report -{" "}
                      {new Date(2000, report.month - 1).toLocaleString(
                        "default",
                        { month: "long" },
                      )}{" "}
                      {report.year}
                ...
                >
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-slate-400 mt-0.5">
                      Grid Size: {scan.gridSize}x{scan.gridSize}
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="text-sm font-bold text-emerald-600">
