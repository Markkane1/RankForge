    ...
    if (job.name === 'UpdateTaskStatus') {
        ...
        await (0, queue_1.withRetry)(async () => {
            ...
            await database_1.prisma.$transaction([
                database_1.prisma.task.update({ where: { id }, data: updateData }),
                database_1.prisma.taskLog.create({
                    data: {
                        taskId: id,
                        level: "INFO",
            ...
            const verify = await database_1.prisma.task.findUnique({ where: { id } });
        ...
        const expiredApprovals = await database_1.prisma.approvalRequest.updateMany({
            where: {
                status: 'PENDING',
        ...
        const credentials = await database_1.prisma.clientCredential.findMany({
        ...
        for (const cred of credentials) {
            if (cred.tokenExpiryAt && cred.tokenExpiryAt < now) {
                await database_1.prisma.clientCredential.update({
                    where: { id: cred.id },
                    data: { isValid: false }
                });
                staleTokens++;
        ...
        for (const cat of mockGoogleTaxonomy) {
            const exists = await database_1.prisma.gbpCategory.findUnique({ where: { name: cat.name } });
            if (exists) {
                if (exists.group !== cat.group) {
                    await database_1.prisma.gbpCategory.update({
                        where: { name: cat.name },
                ...
                await database_1.prisma.gbpCategory.create({
        ...
        const activeProfiles = await database_1.prisma.gbpProfile.findMany({
        ...
        for (const profile of activeProfiles) {
            if (!profile.client?.isActive)
                continue;
            const keywords = profile.client.keywords?.map((k) => k.keyword) || [];
            ...
            for (let week = 1; week <= 4; week++) {
                ...
                await database_1.prisma.gbpPost.create({
                    data: {
        ...
        if (data.messages && data.messages.length > 0) {
            for (const msg of data.messages) {
                ...
                const firstClient = await database_1.prisma.client.findFirst();
                if (firstClient) {
                    await database_1.prisma.leadLogEntry.create({
                        data: {
        ...
        for (const faq of allFaqs) {
            ...
            await database_1.prisma.gbpFaq.update({
                where: { id: faq.id },
                data: {
        ...
        for (const client of activeClients) {
            ...
            for (const kw of priorityKeywords) {
                try {
                    ...
                    await database_1.prisma.geoGridScanResult.create({
                        data: {
                            clientId: client.id,
                            keyword: kw.keyword,
                            gridSize: 3,
            ...
            for (const kw of priorityKeywords) {
                ...
                for (const comp of mockCompetitors) {
                    ...
                    if (matchesKeyword && hasStuffingSign) {
                        ...
                        const exists = await database_1.prisma.task.findFirst({
                            where: {
                                clientId: client.id,
        ...
        for (const client of activeClients) {
            ...
            const currentLeads = await database_1.prisma.leadLogEntry.findMany({
                where: {
                    clientId: client.id,
            ...
            const prevLeads = await database_1.prisma.leadLogEntry.findMany({
                where: {
                    clientId: client.id,
            ...
            const exists = await database_1.prisma.task.findUnique({
