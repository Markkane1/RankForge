...
function deriveKey(): Buffer {
  const raw = requireEnv('TWO_FACTOR_ENCRYPTION_KEY');
...
function getKmsKeyName(): string {
  ...
  if (!projectId || !location || !keyRing || !cryptoKey) {
    throw new Error(
      'GCP_KMS_KEY_NAME or GOOGLE_CLOUD_PROJECT/GCP_KMS_LOCATION/GCP_KMS_KEYRING/GCP_KMS_CRYPTOKEY is required to decrypt kms: secrets in worker',
    );
  }
...
async function withWorkerClientTenant<T>(
  clientId: string,
  fn: (tenantDb: typeof prisma) => Promise<T>,
): Promise<T> {
...
async function recordClientCommunication(
  ...
  await withWorkerClientTenant(client.id, async (tenantDb) => {
    ...
    try {
      await sendStatusAlert(client.email, client.name, message);
      await tenantDb.taskLog.create({
...
async function recordCadenceTask(
  ...
  const writeProof = async (db: typeof prisma) => {
...
async function recordFailedTaskJob(job: { name: string; data?: unknown; attemptsMade: number; opts: { attempts?: number } }, err: Error): Promise<void> {
  ...
  const taskId = (job.data as { id?: unknown }).id;
  ...
  const writeFailure = async (db: typeof prisma) => {
...
async function sendWhatsAppReviewMessage(
  ...
  try {
    const payload = JSON.parse(decrypted) as { accessToken?: string; phoneNumberId?: string };
...
async function processReviewAsk(reviewAskId: string, reminder: boolean): Promise<void> {
  const ask = await prisma.reviewAsk.findUnique({
    where: { id: reviewAskId },
    include: { client: true },
  });
  if (!ask) throw new Error(`ReviewAsk ${reviewAskId} not found`);
  if (ask.optedOut || ask.status === 'OPTED_OUT') return;
  if (!ask.phoneNumber) throw new Error(`ReviewAsk ${reviewAskId} has no phone number`);
...
const worker = new Worker(QUEUE_NAME, async job => {
  ...
  if (job.name === 'UpdateTaskStatus') {
    ...
    await withRetry(async () => {
      ...
      if (status === "DONE" && task.subtasks?.some((st: { isCompleted: boolean }) => !st.isCompleted)) {
      ...
      const applyStatusChange = async (db: typeof prisma) => {
      ...
      if (task.clientId) {
        ...
        await applyStatusChange(prisma);
      }
    ...
    const offboardedClients = await prisma.client.findMany({
    ...
    for (const client of clients) {
      await recordClientCommunication(
        client,
        `Welcome:${client.id}`,
        `Welcome communication: ${client.name}`,
        `Welcome to RankForge. Your onboarding workspace is active and the team has started intake checks.`,
      );
    }
    ...
    for (const client of clients) {
      const taskLogs = await withWorkerClientTenant<WorkerTaskLog[]>(client.id, (tenantDb) =>
      ...
      const message = taskLogs.length
        ? `Weekly BUILD update: ${taskLogs.length} recent task log(s). Latest: ${taskLogs
            .map((log: WorkerTaskLog) => `${log.task.title}: ${log.message}`)
            .join(' | ')}`
      ...
      await recordClientCommunication(
        client,
        `WeeklyBuildSummary:${client.id}:${dateKey}`,
        `Weekly BUILD summary: ${client.name}`,
        message,
      );
    }
    ...
    const since = new Date();
    since.setDate(since.getDate() - 1);
    const changes = await prisma.changeLogEntry.findMany({
    ...
    for (const change of changes) {
      await recordClientCommunication(
        change.client,
        `Milestone:${change.clientId}:${change.newValue}`,
        `Milestone communication: ${change.client.name}`,
        `Milestone reached: your campaign moved to ${change.newValue}.`,
      );
    }
    ...
    for (const client of clients) {
      const report = await withWorkerClientTenant(client.id, (tenantDb) =>
        tenantDb.monthlyReport.findFirst({
      ...
      await recordClientCommunication(
        client,
        `MonthlyReportDelivery:${client.id}:${dateKey}`,
        `Monthly report delivery: ${client.name}`,
        `Your latest monthly RankForge report is ready for review.`,
      );
    }
    ...
    await recordCadenceTask(
      `WeeklyRankUpdate:${dateKey}`,
      'Weekly rank update cadence proof',
      `Weekly rank update checked ${keywords.length} active keyword(s) and detected ${anomaliesDetected} anomaly candidate(s).`,
      { result: { keywordsChecked: keywords.length, anomaliesDetected } },
    );
    ...
    if (!process.env.OPENAI_API_KEY) {
      ...
      await recordCadenceTask(
        `MonthlyPostGenerator:${monthKey}`,
        'Monthly post generator cadence proof',
        'Monthly post generator blocked: content generation is not implemented/configured.',
        { status: 'BLOCKED', level: 'WARN' },
      );
      return;
    }

    const profiles = await prisma.gbpProfile.findMany({
      include: { client: { select: { id: true, name: true, isActive: true } } },
    });
    let draftsCreated = 0;

    for (const profile of profiles.filter((p: { client: { isActive: boolean } }) => p.client.isActive)) {
    ...
    await recordCadenceTask(
      `MonthlyPostGenerator:${monthKey}`,
      'Monthly post generator cadence proof',
      `Monthly post generator created ${draftsCreated} draft post(s) across ${profiles.length} profile(s).`,
      { result: { draftsCreated, rotation: MONTHLY_POST_ROTATION.map((item) => item.eventType) } },
    );
    ...
    if (!process.env.SERP_API_KEY || !process.env.SERP_API_URL) {
      ...
      await recordCadenceTask(
        `FaqVisibilityMonitor:${dateKey}`,
        'FAQ visibility monitor cadence proof',
        'FAQ visibility monitor blocked: SERP_API_KEY and SERP_API_URL are not configured.',
        { status: 'BLOCKED', level: 'WARN' },
      );
      return;
    }
    ...
    await recordCadenceTask(
      `FaqVisibilityMonitor:${dateKey}`,
      'FAQ visibility monitor cadence proof',
      `FAQ visibility monitor tested ${testedCount} FAQ(s).`,
      { result: { testedCount } },
    );
    ...
    for (const client of activeClients) {
      const priorityKeywords = client.keywords.filter((k: WorkerKeyword) => k.priority <= 5);
      const profile = client.gbpProfiles.find((p: WorkerGbpProfile) => p.gbpLocationId);
    ...
    await recordCadenceTask(
      `WeeklyGeoGridScan:${dateKey}`,
      'Weekly geo-grid scan cadence proof',
      `Weekly geo-grid scan saved ${savedScans} scan(s) and skipped ${skippedScans} scan(s).`,
      { result: { activeClients: activeClients.length, savedScans, skippedScans } },
    );
    ...
    for (const client of activeClients) {
      ...
      for (const kw of priorityKeywords) {
        const dataForSeoCredential = await prisma.orgCredential.findFirst({
        ...
        let dataForSeoPayload: any = null;
        try {
          ...
          if (!login || !password) throw new Error('Invalid DataForSEO credential payload');
        ...
        for (const comp of competitors) {
          ...
          if (matchesKeyword && hasStuffingSign) {
            ...
            const exists = await withWorkerClientTenant(client.id, (tenantDb) =>
              tenantDb.task.findFirst({
                where: {
                  clientId: client.id,
    ...
    await recordCadenceTask(
      `MonthlyCompetitorPolicyScan:${dateKey}`,
      'Monthly competitor policy scan cadence proof',
    ...
    for (const client of activeClients) {
      ...
      const prevLeads = await withWorkerClientTenant<WorkerLead[]>(client.id, (tenantDb) =>
        tenantDb.leadLogEntry.findMany({
          where: {
            clientId: client.id,
      ...
      // Group counts
      const currentCalls = currentLeads.filter((l: WorkerLead) => l.source === 'GBP_CALL').length;
      const currentDirections = currentLeads.filter((l: WorkerLead) => l.source === 'GBP_DIRECTIONS').length;
      const currentWebsite = currentLeads.filter((l: WorkerLead) => l.source === 'GBP_WEBSITE').length;

      const prevCalls = prevLeads.filter((l: WorkerLead) => l.source === 'GBP_CALL').length;
      const prevDirections = prevLeads.filter((l: WorkerLead) => l.source === 'GBP_DIRECTIONS').length;
      const prevWebsite = prevLeads.filter((l: WorkerLead) => l.source === 'GBP_WEBSITE').length;
