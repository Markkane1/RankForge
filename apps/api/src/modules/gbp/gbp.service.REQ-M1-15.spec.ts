...
describe('GbpService Backend Validators (REQ-M1-15)', () => {
  ...
  it('should reject invalid phone lengths', async () => {
    await expect(service.updateProfile('client1', { phone: '123' })).rejects.toThrow('Invalid phone number');
  });

  it('should reject invalid URLs', async () => {
    await expect(service.updateProfile('client1', { websiteUrl: 'invalid-url' })).rejects.toThrow('Invalid URL format for website');
  });

  it('should reject ALL-CAPS description', async () => {
    await expect(service.updateProfile('client1', { description: 'THIS IS ALL CAPS' })).rejects.toThrow('ALL-CAPS linter');
  });

  it('should reject exceeding category limits', async () => {
    const categories = Array(10).fill('Cat');
    await expect(service.updateProfile('client1', { secondaryCategories: categories })).rejects.toThrow('Cannot exceed 9 secondary categories');
  });

  it('should reject exceeding service area limits', async () => {
    const areas = Array(21).fill('Area');
    await expect(service.updateProfile('client1', { serviceAreas: areas })).rejects.toThrow('Cannot exceed 20 service areas');
  });
});
